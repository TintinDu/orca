...
const FIXTURES: ShellFixture[] = [
  {
    ...
    secretRead: `print -n 'SECRET> '; read -sk 1\n`,
    childRead: `/bin/zsh -fc "print -n 'CHILD_SECRET> '; read -sk 1"\n`
  },
  {
  ...
  },
  {
    ...
    secretRead: `read -s -n 1 -p 'SECRET> '\n`,
    childRead: `/bin/bash --noprofile --norc -c "read -s -n 1 -p 'CHILD_SECRET> '"\n`
  }
]
...
function startFixture(
  ...
  return {
    ...
    cleanup: async () => {
      if (session.isAlive) {
        try {
          await session.forceKillAndWaitForExit(3_000)
        } catch {
...
async function runReadOracle(fixture: ShellFixture, child: boolean): Promise<void> {
  const prompt = child ? 'CHILD_SECRET> ' : 'SECRET> '
  const running = startFixture(fixture, child ? fixture.childRead : fixture.secretRead)
  try {
    await waitForOutput(running.subscribe, () => running.output().includes(prompt))
  ...
  zshTest(
    ...
    async () => {
      const running = startFixture(
        ...
        `if [[ -z "\${ORCA_EXEC_REPRO_DONE:-}" ]]; then
  export ORCA_EXEC_REPRO_DONE=1
  exec env ZDOTDIR="$HOME" /bin/zsh -l -i
fi
`,
        { '.zshrc': "unset zle_bracketed_paste\nPS1='NO_BRACKET_PROMPT> '\n" }
      )
      try {
